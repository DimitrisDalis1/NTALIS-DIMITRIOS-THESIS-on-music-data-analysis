...
pattern4 = re.compile(r'hpeirwtika|psarantonis|skordalos')
...
for f in fileList:
    if f.endswith(".mp3") and pattern4.match(f):
        mp3list.append(f)
...
for song in ll:
    ...
    singer_d = song[:4]
    print("Singer is: ", singer_d)
    if singer_d == 'hpei':
        newrow['singer'] = 'hpeirwtika'
    elif singer_d == 'psar':
        newrow['singer'] = 'psarantonis'
    elif singer_d == 'skor':
        newrow['singer'] = 'skordalos'
    else:
    # Handle unknown singers (optional)
        newrow['singer'] = 'unknown'
    features_df = pd.concat([features_df, pd.DataFrame([newrow])])


features_df.to_csv("results_features.csv", index=False)
